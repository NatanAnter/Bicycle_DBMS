    ...
    public class SaleDay
    {
        ...
        public static int AddSales(List<Account> AS, DateTime date)
        {
            ...
            foreach (var account in AS)
            {
                ...
                {
                    string str = string.Format("SET DATEFORMAT YMD UPDATE {0} SET  [Amount] = {1} + {2}, Revenues = Revenues + {5}*{2}, Expenses = Expenses + (select Cost from Item where Barcode = '{3}')*{2} WHERE [Barcode] = '{3}' AND [Date] = '{4}'", "Sale", "Amount", account.Amount, account.Barcode, realDate,account.Price);
                    count += DB_Conn.ExecuteNonQuery(str);
                }
                else
                {
                    string str = string.Format("SET DATEFORMAT YMD INSERT INTO Sale VALUES('{0}', '{1}', {2}, {3}, (select Cost from Item where Barcode = '{0}')*{2},N'{4}');", account.Barcode, realDate, account.Amount, account.Price*account.Amount,account.Name);
                    count += DB_Conn.ExecuteNonQuery(str);
                }
            }
            return count;
